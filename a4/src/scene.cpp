...

{
  ...
  if (g == 1 || numRaySamples == 1) {
    ...
    float theta = acos(g);      // half angle of the cone
    float L = 0.5f / tan(theta);    // distance to the unit circle on cone 

    // two unit vectors perpendicular to the ideal reflection 
    vec3 perp1 = R.perp1().normalize();
    vec3 perp2 = R.perp2().normalize();

    vec3 rayIout = vec3(0, 0, 0); // variable to hold the accumulated light of the sample rays

    for (int i = 0; i < numRaySamples; i++) {

        // rejection sampling within the disc
        float a = 1.0;
        float b = 1.0;
        while (((a * a) + (b * b)) > 1) {
            a = randIn01();
        ...
        // find sample rays light and add it to the total light
        vec3 sampleRay = ((L * R) + (a * perp1) + (b * perp2)).normalize(); // random sample direction
        vec3 sampleIin = raytrace(P, sampleRay, depth, objIndex, objPartIndex); // light recieved from the sample ray
        rayIout = rayIout + calcIout(N, R, E, E, kd, mat->ks, mat->n, sampleIin); 
    }

    // average the light from all sample rays
    rayIout = (1.0f/numRaySamples) * rayIout;
    Iout = Iout + rayIout;
  ...
  for (int i=0; i<objects.size(); i++) {
	...
	// ---------------- BEGIN YOUR CODE HERE ----------------
          ...
          for (int j = 0; j < numRaySamples; j++) {

              // rejection sampling within the area light
              float a = 1.0;
              float b = 1.0;
              while ((a+b) > 1) {
                  a = randIn01();
              ...
              // find point with barycentric coordinates
              vec3 v0 = tri->verts[0].position;
              vec3 v1 = tri->verts[1].position;
              vec3 v2 = tri->verts[2].position;
              vec3 pointOnLight = v0 + a*(v1 - v0) + b*(v2 - v0);

              vec3 L = pointOnLight - P; // direction to light

              // is light above the surface?
              if (N * L > 0) {

                  float  Ldist = L.length(); // distance to light
                  L = (1.0 / Ldist) * L;    // normalized
                  ...
                  // is there an object between P and the light?
                  bool hit = findFirstObjectInt(P, L, objIndex, objPartIndex, intP, intN, intTexCoords, intT, intObjIndex, intObjPartIndex, intMat, i);

                  // if there is an object and is that object the emitting triangle?
                  //if (intT < Ldist + 0.01 && intT > Ldist - 0.01){ // check using distance 
                  if (hit && intObjIndex == i){  // check using object index
                     vec3 Lr = (2 * (L * N)) * N - L;
                     Iout = Iout + calcIout(N, L, E, Lr, kd, mat->ks, mat->n, (1.0f/numRaySamples)*(tri->mat->Ie));
                  }
              }
          }
	// ---------------- END YOUR CODE HERE ----------------
...

{
  ...
  // Calculate the size of each subpixel
  float subpixelSize = 1.0f / numPixelSamples;
  ...
  for (int i = 0; i < numPixelSamples; i++) {
      for (int j = 0; j < numPixelSamples; j++) {
          ...
          if (jitter) {
              subpixelX = x - 0.5f + (i + randIn01()) * subpixelSize;
              subpixelY = y - 0.5f + (j + randIn01()) * subpixelSize;
          }
          else {
              subpixelX = x - 0.5f + (i + 0.5f) * subpixelSize;
              subpixelY = y - 0.5f + (j + 0.5f) * subpixelSize;
          }

          // Calculate the direction of the ray from the eye to the subpixel
          vec3 dir = (llCorner + subpixelX * right + subpixelY * up).normalize();

          // get sub pixel colour
          vec3 subpixelColour = raytrace(eye->position, dir, 0, -1, -1);

          // Accumulate all sub pixels colour
          result = result + subpixelColour;
      }
  }

  // Average the colour
  result = (1.0f / (float)(numPixelSamples * numPixelSamples)) * result;
