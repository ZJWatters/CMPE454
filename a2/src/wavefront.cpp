...

{
  ...
  while(fscanf(file, "%s", buf) != EOF) {
	...
	if (buf[strlen(buf)-1] == '/' && buf[strlen(buf)-2] == '/' ) {	/* v// */

	  numV++;

	  sscanf(buf,  "%d//", &v); v--; checkVindex(v); tri->vindices[0] = v;
	  fscanf(file, "%d//", &v); v--; checkVindex(v); tri->vindices[1] = v;
	  fscanf(file, "%d//", &v); v--; checkVindex(v); tri->vindices[2] = v;
  ...
  // Add vertex normals if necessary (very expensive)

  if (!hasVertexNormals)  {

    normals = seq<vec3>( vertices.size(), vec3(0,0,0) );
	...
	wfTriangle &tri = *groups[g]->triangles[i];
	for (int k=0; k<3; k++) {
	  int idx = tri.vindices[k];
	  normals[idx] = normals[idx] + facetnorms[ tri.findex ];
	  tri.nindices[k] = idx;
	}
      }
...
public:
  ...
  bool operator == (const VertexSignature &p) const {
    return sig[0] == p.sig[0] && sig[1] == p.sig[1] && sig[2] == p.sig[2];
  }
};

namespace std {
  template <> struct hash<VertexSignature> {
    size_t operator()(const VertexSignature& vs) const {
      return (((size_t) vs.sig[0]) << 48) | (((size_t) vs.sig[1]) << 24) | ((size_t) vs.sig[0]);  // relies on sizeof(size_t) == 8
    }
  };
}
...

{
  ...
  for (int i=0; i<groups.size(); i++)
    n += groups[i]->triangles.size();

  // Process each group separately

  std::unordered_map< VertexSignature, GLuint > hash;
  ...
  for (int i=0; i<groups.size(); i++) {
	...
	for (int k=0; k<3; k++) {
	  ...
	  auto search = hash.find( vs );
	  
	  if (search != hash.end()) // found 

	    faceIndexBuffer[ nFaces * 3 + k ] = search->second; // Store this vertex index

	  else { // not found: add this vertex
		...
		* (vec2*) &vertexBuffer[nVerts*vertexSize+3] = * (vec2*) &texcoords[ tri->tindices[k] ];
	    ...
	    vertSig[ nVerts ] = vs;
	    hash.insert( { vs, nVerts } ); // nVerts is the index of this vertex
	    faceIndexBuffer[ nFaces * 3 + k ] = nVerts; // Store this vertex index
	    nVerts++;
	  }
	}
