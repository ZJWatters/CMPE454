...

{
    ...
    for (int i = 0; i < nSpheres; i++) {
        y[i] = yStart[i];
        yDeriv[i].x.x = y[i].v.x;
        yDeriv[i].x.y = y[i].v.y;
        yDeriv[i].x.z = y[i].v.z;
    ...
    for (int i = 0; i < nSpheres; i++) {
        yEnd[i].x.x = y[i].x.x + (deltaT * yDeriv[i].x.x);
        yEnd[i].x.y = y[i].x.y + (deltaT * yDeriv[i].x.y);
        yEnd[i].x.z = y[i].x.z + (deltaT * yDeriv[i].x.z);
        ...
        yEnd[i].v.x = y[i].v.x + (deltaT * yDeriv[i].v.x); 
        yEnd[i].v.y = y[i].v.y + (deltaT * yDeriv[i].v.y);
        yEnd[i].v.z = y[i].v.z + (deltaT * yDeriv[i].v.z);
...

{
    ...
    if (!collisionAtEnd) {
        ...
        // a collision: Do binary search until interval is at most
        ...
        float interval = deltaT / 2.0f;


        while (MAX_TIME_STEP > actualDeltaT && interval > deltaT/128.0) { // 1/280 th of deltaT 
            integrate(yStart, yEnd, interval, collisionAtEnd, &collisionSphere, &collisionObject);
    
            if (collisionAtEnd) {                // collision in first half
                copyState(yStart, &spheres[0]);  // copy original start state 
                interval = interval / 2.0; 
            }
            else {                          // collision in 2nd half
                copyState(yEnd, yStart);    // copy end state of last integration into the start of next and integrate w same time interval
                actualDeltaT += interval;   // moved to the end time of the last integration
            }
        }
...

{
    ...
    if (otherIsSphere) {
        ...
        // vector connecting the sphere centres normalized
        ...
        // sphere velocities done following code steps 
        sphere->state.v = v1a * n;  // sphere 1 velocity AFTER
        sphere2->state.v = v2a * n;  // sphere 2 velocity AFTER

        /* sphere velocities done using components
        // spheres perpendicular and parrallel components
        vec3 v_perp1 = v1b * n;
        vec3 v_para1 = sphere->state.v - v_perp1;
        vec3 v_perp2 = v2b * n;
        vec3 v_para2 = sphere2->state.v - v_perp2;

        sphere->state.v = v_para1 + (COEFF_OF_RESTITUTION * v_perp1);
        sphere2->state.v = v_para2 + (COEFF_OF_RESTITUTION * v_perp2);
        */
        ...
        vec3 contactPoint;
        float dist = sphere->distToRectangle( *rectangle, &contactPoint);
        vec3 n = (sphere->state.x - contactPoint).normalize();
        ...
        // find perpendicular and parallel components of the velocity wrt the normal
        vec3 v_perp = v1b * n;
        vec3 v_para = sphere->state.v - v_perp;

        sphere->state.v = v_para + (COEFF_OF_RESTITUTION * v_perp);
