...

{
  ...
  float minX = - rectangle.xDim / 2;
  float maxX = rectangle.xDim / 2;
  float minY = - rectangle.yDim / 2;
  float maxY = rectangle.yDim / 2;

  if ((sphereCentre.x < maxX) && (sphereCentre.x > minX) && (sphereCentre.y < maxY) && (sphereCentre.y > minY)) {  
    *closestPoint = (Minv.inverse() * vec4(sphereCentre.x, sphereCentre.y, 0.0f, 1.0f)).toVec3();
    return (fabs(sphereCentre.z) - this->radius); //radius = 0.1
  }
  ...
  distYplus = pointToEdgeDistance(sphereCentre, vec3(minY, maxY, 0.0), vec3(maxX, maxY, 0.0), &pointYplus);
  ...
  *closestPoint = (Minv.inverse() * vec4(pt, 1.0f)).toVec3();
